...
isOnCooldown = function(cmd)
    ...
    local function createPlayerRow(plr)
        ...
        -- ── Highlight row red if this player owns the base we're currently inside (ported from Bullys Hub) ──
        ...
        task.spawn(function()
            while row and row.Parent do
                ...
                pcall(function()
                        ...
                        if Plots then
                            ...
                            for _, plot in ipairs(Plots:GetChildren()) do
                                ...
                                if ok2 and pivot then
                                    local dist = (localHRP.Position - pivot.Position).Magnitude
                                    local size = plot:GetExtentsSize()
                                    local radius = (size.Magnitude / 2) * 1.2
                            ...
                            if bestPlot then
                                -- Try Synchronizer first
                                local okSync, ch = pcall(function()
                                    return Synchronizer:Get(bestPlot.Name)
                                end)
                                if okSync and ch then
                                    local owner = ch:Get("Owner")
                                    if owner then
                                        if typeof(owner) == "Instance" and owner:IsA("Player") then
                                            isOwner = (owner.UserId == plr.UserId)
                                        elseif type(owner) == "table" and owner.UserId then
                                            isOwner = (owner.UserId == plr.UserId)
                                        elseif type(owner) == "table" and owner.Name then
                                            isOwner = (owner.Name == plr.Name)
                                        end
                                    end
                                end
                                -- Fallback: check PlotSign text
                                if not isOwner then
                                    local sign = bestPlot:FindFirstChild("PlotSign")
                                    ...
                                    if tl then
                                        local nickname = tl.Text and tl.Text:match("^(.-)'") or tl.Text
                                        if nickname then
                                            isOwner = (plr.DisplayName == nickname or plr.Name == nickname)
                                        end
                                    end
                                end
                            end
                    ...
                    if isOwner then
                        row.BackgroundColor3 = Color3.fromRGB(90, 25, 25)
                        rowStroke.Color = Color3.fromRGB(220, 80, 80)
...
-- ── BRAINROT IMAGE ON BASE (ported from Bullys Hub) ───────────────────────
task.spawn(function()
    ...
    local function isBrainrotTool(toolName)
        local n = (toolName or ""):lower()
        return n ~= "flying carpet"
            and n ~= "quantum cloner"
            and n ~= "bat"
            and n ~= "glove"
    end
    ...
    local function getBasePart()
        ...
        return myPlot:FindFirstChild("MainRootPart")
            or myPlot:FindFirstChildWhichIsA("BasePart")
    end

    local function showBrainrotOnBase(toolName)
        local basePart = getBasePart()
        if not basePart then return end

        -- Remove any existing brainrot display
        local existing = basePart:FindFirstChild("BrainrotBaseGui")
        if existing then existing:Destroy() end

        if not toolName or toolName == "" then return end

        local imageUrl = getBrainrotImageId(toolName)
        if imageUrl == "" then return end

        local surfaceGui = Instance.new("SurfaceGui")
        surfaceGui.Name = "BrainrotBaseGui"
    ...
    while true do
        ...
        for _, tool in ipairs(char:GetChildren()) do
            if tool:IsA("Tool") and isBrainrotTool(tool.Name) then
                currentBrainrot = tool.Name
                break
            end
        end

        if currentBrainrot ~= lastBrainrot then
            lastBrainrot = currentBrainrot
            showBrainrotOnBase(currentBrainrot) -- nil = clears the image
        end
    end
end)
-- ── STEALING PLOT ESP (shows stolen brainrots/pets from bully → lefa) ────────
task.spawn(function()
    local espGui = Instance.new("ScreenGui")
    espGui.Name = "LefaStealingPlotESP"
